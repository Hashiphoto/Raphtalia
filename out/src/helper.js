...
function reportInfractions(member, channel, pretext = "") {
    ...
    return db_js_1.default.users.get(member.id, member.guild.id).then((user) => {
        let reply;
...
function pardon(member, channel) {
    db_js_1.default.users.setInfractions(member.id, member.guild.id, 0);
...
function exile(member, channel, releaseDate = null) {
    ...
    let message = "";
    if (releaseDate != null) {
        let duration = releaseDate.diff(dayjs_1.default());
        if (duration > 0x7fffffff) {
            duration = 0x7fffffff;
            releaseDate = dayjs_1.default().add(duration, "ms");
        }
        let timerId = setTimeout(() => {
            pardon(member, channel);
        }, duration);
        clearExileTimer(member);
 ...
 */
function setHoistedRole(member, role) {
    return __awaiter(this, void 0, void 0, function* () {
        let discordRole = convertToRole(member.guild, role);
        let dbRole = yield db_js_1.default.roles.getSingle(discordRole.id);
...
function convertToRole(guild, roleResolvable) {
    ...
    if (roleResolvable instanceof discord_js_1.default.Role) {
...
function checkInfractionCount(channel, member, count = null) {
    return __awaiter(this, void 0, void 0, function* () {
        if (count == null) {
            let user = yield db_js_1.default.users.get(member.id, member.guild.id);
...
function promote(channel, sender, target) {
    ...
    if (sender != null) {
        if (sender.id === target.id) {
            addInfractions(sender, channel, 1, links_json_1.default.gifs.bernieNo);
            return;
        }
    ...
    setHoistedRole(target, nextHighest).then(([roleChanged, memberLimit]) => {
        if (roleChanged) {
            ...
            channel.send(`Cannot promote because ${nextHighest.name} already has ${nextHighest.members.size}/${memberLimit} members!`);
        }
    });
}
/**
...
function demote(channel, sender, target) {
    ...
    if (nextLowest.id == discordConfig.roles.exile) {
        exile(target, channel, dayjs_1.default().add(1, "day"));
        return;
    }
    // demote the target
    ...
    setHoistedRole(target, nextLowest).then(([roleChanged, memberLimit]) => {
        if (roleChanged) {
            ...
            channel.send(`Cannot demote because ${roleName} already has ${nextLowest.members.size}/${memberLimit} members!`);
        }
    });
}
/**
...
function addCurrency(member, amount = 1) {
    return db_js_1.default.users.incrementCurrency(member.id, member.guild.id, amount);
}
function getLeaderRole(guild) {
    return guild.o.roles
...
function getUserIncome(member) {
    return db_js_1.default.users.get(member.id, member.guild.id).then((dbUser) => {
        ...
        return db_js_1.default.roles.getSingle(member.hoistRole.id).then((dbRole) => {
            if (!dbRole) {
                return bonusIncome;
            }
