...
import { addInfractions, setInfractions } from "./MemberController.js";
...
class RoleController {
  ...
  pardonMember(member, channel) {
    ...
    if (hasRole(member, discordConfig().roles.exile)) {
      ...
      if (channel)
        channel.watchSend(`${member} has been cleared of all charges`);
    }
  }
   ...
   * @param {*} roleId
   */
  convertToRole(guild, roleResolvable) {
  ...
  promoteMember(channel, sender, target) {
    ...
    setHoistedRole(target, nextHighest).then(([roleChanged, memberLimit]) => {
      if (roleChanged) {
        channel.watchSend(
          `${target} has been promoted to ${nextHighest.name}!`
        );
      } else {
        channel.watchSend(
   ...
   * @param {Discord.GuildMember} target - The GuildMember being promoted
   */
  demoteMember(channel, sender, target) {
