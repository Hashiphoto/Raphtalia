...
class MemberController extends GuildBasedController {
   ...
   */
  addInfractions(member, channel, amount = 1, reason = "") {
    db.users
   ...
   * @param {String} [reason] - A message to append to the end of the infraction notice
   */
  setInfractions(member, channel, amount = 1, reason = "") {
    db.users
  ...
  // TODO: Rework so that messages are not sent in the controller
  /**
   ...
   */
  reportInfractions(member, channel, pretext = "") {
   ...
   */
  async checkInfractionCount(channel, member, count = null) {
    if (count == null) {
