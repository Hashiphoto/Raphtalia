...
function censorMessage(message) {
	return db.guilds.get(message.guild.id).then((guild) => {
		...
		if (
			helper.hasRole(sender, discordConfig().roles.leader) ||
			helper.hasRole(sender, discordConfig().roles.immigrant)
		) {
			return false;
		}
...
function banWords(channel, sender, words, permissionLevel) {
	return db.guilds.get(sender.guild.id).then((guild) => {
		if (!guild.censorship_enabled) {
		...
		if (!helper.verifyPermission(sender, channel, permissionLevel)) {
			return;
		}
...
function allowWords(channel, sender, words, permissionLevel) {
	return db.guilds.get(sender.guild.id).then((guild) => {
		...
		db.bannedWords.delete(sender.guild.id, words).then(() => {
			rebuildCensorshipList(sender.guild.id);
		});
		if (channel)
