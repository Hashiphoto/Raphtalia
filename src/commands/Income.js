  ...
  async execute() {
    if (!this.message.args || this.message.args.length === 0) {
      return getUserIncome(this.message.sender).then((income) => {
        return this.inputChannel.watchSend(`Your daily income is $${income.toFixed(2)}`);
      });
    }
    ...
    let baseIncome = (await this.db.guilds.get(this.message.guild.id)).base_income;
    ...
    for (let i = 0; i < this.message.args.length - 1; i++) {
      ...
      if (amount.isPercent && amount.number < 1) {
        return this.inputChannel.watchSend("Income scale must be at least 100%");
      }
